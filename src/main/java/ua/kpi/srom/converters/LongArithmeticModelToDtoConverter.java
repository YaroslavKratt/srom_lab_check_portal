...
@Component
public class LongArithmeticModelToDtoConverter
    implements Converter<LongArithmeticModel, LongArithmeticDto> {
  @Override
  ...
  private String mapIfNotNull(BigInteger number, int numberSystem) {
    return Optional.ofNullable(number).map(n -> n.toString(numberSystem)).orElse(null);
  }
}
